...
const ToggleSwitch = ({ selected, onSelect, iconLeft, iconRight }) => {
  const insetPx = 4;
...
const PriceSelector = ({ options = [], valueIndex = 0, onChange }) => {
  const insetPx = 4;
...
const DonationSummary = ({ selected = 'once', amount = '', priceIndex = 0, priceOptions = [], detailLines = [] }) => {
  const amt = parseFloat(String(amount).replace(/[^0-9.]/g, '')) || (priceOptions[priceIndex] ?? 0);
  const formatted = (n) => (isNaN(n) ? '0.00' : n.toFixed(2));
  const yearly = amt * 12;
...
const DonationCalculator = ({ title, children, footer, iconLeft, iconRight, onToggle }) => {
